...
uint8_t inactiveFontColor = TS_8b_Gray;
uint8_t inactiveFontBG = TS_8b_Black;
uint8_t positiveFontColor = TS_8b_Green;
uint8_t negativeFontColor = TS_8b_Red;
...
void loop() {
  ...
  if (cyclingModeState == 1) { cyclingMode(); }
  checkButtons();
}

void cyclingMode() {
  // please put your fall detector here
  crashUI();
  cyclingModeState = 0; // to be removed after integrating the fall detector
}
